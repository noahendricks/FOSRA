...
function MessageEditing({ content, onSubmitEdit, onCancelEdit }: MessageEditingProps) {
  ...
  return (
    <div className="w-full">
      <div
        className={cn("w-full h-full border rounded-16 overflow-hidden p-3 flex flex-col gap-2")}
      >
        <textarea
          ref={textareaRef}
          className={cn(
...
function arePropsEqual(prev: HumanMessageProps, next: HumanMessageProps): boolean {
  return (
...
const HumanMessage = React.memo(function HumanMessage({
  ...
  const currentMessageInd =
    indexInSiblings !== undefined && indexInSiblings !== -1 ? indexInSiblings : undefined;

  const getPreviousMessage = () => {
    if (currentMessageInd !== undefined && currentMessageInd > 0 && otherMessagesCanSwitchTo) {
  ...
  return (
    <Hoverable.Root group="humanMessage" width="full">
      <div id="onyx-human-message" className="flex flex-col justify-end w-full relative">
        ...
        {isEditing ? (
          ...
          <div className="flex justify-end">
            ...
            <div className="md:max-w-[37.5rem]">
              ...
              >
                <Text as="p" className="inline-block align-middle" mainContentBody>
                  {content}
                </Text>
              </div>
            </div>
