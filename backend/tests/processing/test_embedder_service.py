...
from backend.src.config.request_context import EmbedderConfig
...
class TestSentenceTransformersEmbedder:
    ...
    async def test_embed_texts(self, sample_texts: list[str], mock_request_context):
        ...
        with patch(
            "backend.src.processing.services.embedder_service.asyncio.to_thread"
        ) as mock_to_thread:
...
class TestOpenAIEmbedder:
    ...
    async def test_embed_texts_success(self, sample_texts: list[str], mock_session):
        ...
        with patch(
            "backend.src.processing.services.embedder_service.openai.AsyncOpenAI"
        ) as mock_openai:
...
class TestCohereEmbedder:
    ...
    async def test_embed_texts_success(self, sample_texts: list[str], mock_session):
        ...
        with patch(
            "backend.src.processing.services.embedder_service.cohere.AsyncClient"
        ) as mock_cohere:
