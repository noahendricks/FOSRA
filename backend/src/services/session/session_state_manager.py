...
class SessionStateManager:
    ...
    def __init__(self):
        self._repo: SessionStateRepo | None = None
    ...
    def set_repo(self, repo: SessionStateRepo):
        self._repo = repo
    ...
    async def get(self, session_id: str) -> dict[str, Any] | None:
        ...
        if self._repo:
            orm = await self._repo.get(session_id)
            if orm:
                state = {
    ...
    async def list_active(self, limit: int = 50) -> list[dict[str, Any]]:
        if self._repo:
            orm_states = await self._repo.list_active(limit)
            return [
                {
