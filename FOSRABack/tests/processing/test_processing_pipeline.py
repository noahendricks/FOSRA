...
from FOSRABack.src.processing.services.processing_pipeline import (
    ChunkingStrategy,
    ProcessingConfig,
    ProcessingPipeline,
    ProcessingResult,
...
class TestProcessDocumentSuccess:
    ...
    async def test_process_document_basic(
        ...
        with patch.object(
            ...
            with patch(
                "FOSRABack.src.processing.services.chunker_service.ChunkerService"
            ) as MockChunker:
    ...
    async def test_process_document_with_summary(
        ...
        with patch.object(
            ...
            with patch(
                ...
                with patch(
                    "FOSRABack.src.convo.services.llm_service.LLMService.get_llm_for_role",
                    return_value=MagicMock(),
                ):
                    with patch(
                        "FOSRABack.src.processing.utils.processing_utils.generate_document_summary",
...
class TestCreateSourceWithChunks:
    ...
    async def test_create_source_with_chunks(
        ...
        with patch(
            "FOSRABack.src.processing.utils.processing_utils.generate_content_hash",
            return_value="content_hash_123",
        ):
            with patch(
                "FOSRABack.src.processing.utils.processing_utils.generate_unique_identifier_hash",
