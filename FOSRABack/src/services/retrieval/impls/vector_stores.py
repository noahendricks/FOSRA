...
from FOSRABack.src.domain.exceptions import InfrastructureError, VectorStorageError
...
from FOSRABack.src.domain.enums import VectorStoreType
...
@BaseVectorStore.register(VectorStoreType.WEAVIATE)
class WeaviateVectorStore(APIVectorStore):
    store_type: ClassVar[VectorStoreType] = VectorStoreType.WEAVIATE
    display_name: ClassVar[str] = "Weaviate"
    ...
    async def initialize(self, config: VectorStoreConfig) -> None:
        try:
            ...
            #         http=ProtocolParams(host=""),  # pyright: ignore
            #         auth_client_secret=auth,  # pyright:ignore
            #     )
            # )
            self._initialized = True
    ...
    async def ensure_collection(
        ...
        def create_class():
            if not self.client.schema.exists(collection_name):  # type: ignore
                class_obj = {
                ...
                self.client.schema.create_class(class_obj)  # type: ignore
                logger.info(f"Created Weaviate class: {collection_name}")

        await loop.run_in_executor(None, create_class)
    ...
    async def upsert(
        ...
        if not self._initialized or not sources:
            return []
        ...
        await loop.run_in_executor(None, batch_upsert)
        ...
        logger.info(f"Upserted {len(chunk_ids)} vectors to Weaviate")
        return chunk_ids
    ...
    async def search(
        ...
        def do_search():
            query = (
                self.client.query.get(config.collection_name, ["*"])  # pyright: ignore  # type: ignore
        ...
        search_results = []
        for item in (
            results.get("data", {}).get("Get", {}).get(config.collection_name, [])
        ):
    ...
    async def delete(self, collection_name: str, point_ids: list[str]) -> int:
        ...
        if not self._initialized or not point_ids:
            return 0
        ...
        def do_delete():
            for pid in point_ids:
                self.client.data_object.delete(pid, class_name=collection_name)  # type: ignore

        await loop.run_in_executor(None, do_delete)
        logger.info(f"Deleted {len(point_ids)} points from Weaviate")
        return len(point_ids)
    ...
    async def get_collection_info(self, collection_name: str) -> dict[str, Any]:
        ...
        def get_info():
            schema = self.client.schema.get(collection_name)  # type: ignore
            count = self.client.query.aggregate(collection_name).with_meta_count().do()  # type: ignore
            return schema, count

        schema, count = await loop.run_in_executor(None, get_info)
