  ...
  init: () => {
    ...
    const agent = iife(() => {
      const agents = createMemo(() =>
        store.state.agents().filter((x) => x.mode !== "subagent" && !x.hidden),
      );
      const visibleAgents = createMemo(() =>
        store.state.agents().filter((x) => !x.hidden),
      );
      ...
      return {
        ...
        move(direction: 1 | -1) {
          batch(() => {
            ...
            let next =
              list.findIndex((x) => x.name === agentStore.current) + direction;
    ...
    const model = iife(() => {
      ...
      Filesystem.readJson(filePath)
        .then((x: any) => {
          if (typeof x.model === "object" && !Array.isArray(x.model))
            setModelStore("model", x.model);
          if (Array.isArray(x.recent)) setModelStore("recent", x.recent);
          if (Array.isArray(x.favorite)) setModelStore("favorite", x.favorite);
          if (typeof x.variant === "object")
            setModelStore("variant", x.variant ?? {});
        })
        .catch(() => {})
        .finally(() => {
      ...
      return {
        ...
        cycle(direction: 1 | -1) {
          ...
          log.provider.info("MODEL_CYCLED", {
            direction,
            model: current,
            recent: modelStore.recent,
          });
        ...
        cycleFavorite(direction: 1 | -1) {
          const favorites = modelStore.favorite.filter((item) =>
            isModelValid(item),
          );
          if (!favorites.length) {
            toast.show({
          ...
          save();
          log.provider.info("MODEL_FAVORITE_CYCLED", {
            direction,
            model: next,
            favorites: modelStore.favorite,
          });
        },
        set(
        ...
        toggleFavorite(model: { providerID: string; modelID: string }) {
          ...
          log.provider.info("MODEL_FAVORITE_TOGGLED", {
            model,
            favorited: !modelStore.favorite.some(
              (x) =>
                x.providerID === model.providerID &&
                x.modelID === model.modelID,
            ),
        ...
        variant: {
          ...
          set(value: string | undefined) {
            ...
            save();
            log.provider.debug("MODEL_VARIANT_SET", {
              variant: value,
              model: m,
            });
          },
          cycle() {
            ...
            if (!current) {
              this.set(variants[0]);
              log.provider.debug("MODEL_VARIANT_CYCLED", {
                variant: variants[0],
                model: currentModel(),
                variants,
              });
    ...
    const mcp = {
      ...
      async toggle(name: string) {
        ...
        log.mcp.info("MCP_TOGGLED", {
          name,
          enabled: willEnable,
          status: store.state.mcp.get(name),
        });
      },
    };
