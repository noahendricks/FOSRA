...
export const createFosraClient = (options: {
  ...
  async function api<T>(
    path: string,
    init?: { method?: string; body?: string; throwOnError?: boolean; signal?: AbortSignal },
  ): Promise<Result<T>> {
    ...
    if (!res.ok) {
      ...
      return { error: { code: String(res.status), message: msg }, response: res }
    }
  ...
  return {
    event: {
      subscribe: async (_params?: any, _opts?: any): Promise<StreamResult<Event>> => {
    ...
    config: {
      providers: async (_p?: any, o?: any) => api<any>("/config/provider", opts(o)),
      get: async (_p?: any, o?: any) => api<any>("/config", opts(o)),
      update: async (p?: any, o?: any) =>
        api<any>("/config", { method: "PUT", body: JSON.stringify(p), ...opts(o) }),
    },
    provider: {
      list: async (_p?: any, o?: any) => api<ProviderListResponse>("/provider", opts(o)),
      auth: async (_p?: any, o?: any) => api<any>("/provider/auth", opts(o)),
      oauth: {
        authorize: async (_p?: any, _o?: any) =>
          mock<ProviderAuthAuthorization>({ url: "", method: "auto", instructions: "" }),
    ...
    session: {
      ...
      get: async (p: any, o?: any) => api<Session>(`/session/${p.sessionID}`, opts(o)),
      create: async (p?: any, o?: any) =>
        api<Session>("/session", { method: "POST", body: JSON.stringify(p), ...opts(o) }),
      messages: async (p: any, o?: any) => {
        const query = p?.limit ? `?limit=${p.limit}` : ""
        return api<Array<{ info: Message; parts: Part[] }>>(`/session/${p.sessionID}/message${query}`, opts(o))
      },
      todo: async (p: any, o?: any) => api<any[]>(`/session/${p.sessionID}/todo`, opts(o)),
      diff: async (p: any, o?: any) => api<any[]>(`/session/${p.sessionID}/diff`, opts(o)),
      ...
      shell: async (p: any, o?: any) => {
        const { sessionID, ...body } = p
        return api<any>(`/session/${sessionID}/shell`, { method: "POST", body: JSON.stringify(body), ...opts(o) })
      },
      command: async (p: any, o?: any) => {
        const { sessionID, ...body } = p
        return api<any>(`/session/${sessionID}/command`, { method: "POST", body: JSON.stringify(body), ...opts(o) })
      },
      update: async (p: any, o?: any) => {
        const { sessionID, ...body } = p
        return api<Session>(`/session/${sessionID}`, { method: "PUT", body: JSON.stringify(body), ...opts(o) })
      },
      delete: async (p: any, o?: any) =>
        api<boolean>(`/session/${p.sessionID}`, { method: "DELETE", ...opts(o) }),
    ...
    formatter: {
      status: async (_p?: any, o?: any) => api<any[]>("/formatter/status", opts(o)),
    },
    vcs: {
