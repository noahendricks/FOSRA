...
export const Message = ({ className, role, ...props }: MessageProps) => (
  <div
    className={cn(
      "group flex w-full max-w-[95%] flex-col gap-2",
      role === "user" ? "is-user ml-auto justify-end-safe" : "is-assistant",
      className,
    )}
    {...props}
  />
...
export const MessageActions = ({
  ...
  <div
    className={cn(
      messageRole === "assistant" && "flex items-center gap-1",
      messageRole === "user" && "flex items-center gap-1 justify-end-safe",
      className,
    )}
    {...props}
  >
...
MessageBranch.displayName = "MessageBranch"; // ✅ Add this
...
export const MessageBranchContent = ({
  ...
  useEffect(() => {
    if (branches.length !== validChildren.length) {
      setBranches(validChildren);
    }
  }, [validChildren.length, branches.length, setBranches, validChildren]); // ✅ Use length instead of array

  return validChildren.map((branch, index) => (
    <div
      className={cn(
        "grid gap-2 overflow-hidden [&>div]:pb-0",
        index === currentBranch ? "block" : "hidden",
      )}
      key={branch.key ?? index} // ✅ Fallback to index if no key
      {...props}
    >
      {branch}
    </div>
